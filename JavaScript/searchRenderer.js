document.addEventListener('DOMContentLoaded', () => {
  ...
  function ensureSearchUI() {
    ...
    let pagerDiv = document.getElementById('searchPager');
    let timeDiv = document.getElementById('searchTime');
    ...
    if (!input || !searchBtn) {
      ...
      wrapper.style.cssText='display:flex;gap:6px;margin-top:10px;';

      input = document.createElement('input');
      input.id='searchInput'; input.type='search'; input.placeholder='Search games';
      input.style.flex='1'; wrapper.appendChild(input);
    ...
    return { input, searchBtn, resultsDiv, pagerDiv, timeDiv };
  }

  const ui = ensureSearchUI();
  const { input, searchBtn, resultsDiv, pagerDiv, timeDiv } = ui;
  ...
  function renderPage(page=1){
    ...
    for(let i=start;i<end;i++){
      ...
      img.src=g.headerImage||PLACEHOLDER; img.width=120; img.height=70;
    ...
    function addBtn(p){
      ...
      b.style.cssText=`padding:4px 8px;border-radius:4px;border:1px solid #888;background:${p===currentPage?'#66c0f4':'#152d3a'};color:${p===currentPage?'#07141d':'#ccc'};cursor:pointer;`;
      if(p!==currentPage) b.addEventListener('click',()=>renderPage(p));
